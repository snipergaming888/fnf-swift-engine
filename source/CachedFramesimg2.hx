...
class CachedFramesimg2
{
    public static var cachedInstance:CachedFramesimg2;

    function new() {}

    public static function loadEverything()
    {
        cachedInstance = new CachedFramesimg2();
    ...
    public function fromSparrow(id:String, xmlName:String)
    {
		...
		var Description = Assets.getText(Paths.file('images/$xmlName.xml', 'week2'));
    ...
    {
        var graph = FlxGraphic.fromAssetKey(Paths.image(path,'week2'));
        graph.persist = true;
    ...
    public function loadFrames()
    {
        sys.thread.Thread.create(() -> {
            toBeLoaded.set('halloween_bg','halloween_bg');
            toBeLoaded.set('halloween_bg_opt','halloween_bg_opt');
            toBeLoaded.set('Monster_Assets','Monster_Assets');
            toBeLoaded.set('spooky_kids_assets','spooky_kids_assets');
